    ...
    try {
        ...
        sessionTypes.forEach(st => {
            ...
            byCategory[category].push({
                ...
                categoryObj: st.Category,
                subcategoryObj: st.Subcategory,
                programId: st.ProgramId,
                programObj: st.Program
            });
        });
...
app.get('/api/session-types', async (req, res) => {
    try {
        ...
        let programs = {};
        let onlineBookablePrograms = new Set(); // Track which programs allow online booking
        try {
            ...
            if (programsData.Programs) {
                programsData.Programs.forEach(prog => {
                    ...
                    // Check if program is available for online booking
                    if (prog.ScheduleType === 'Appointment' && prog.ContentFormats?.includes('Online')) {
                        onlineBookablePrograms.add(prog.Id);
                    }
                    // Also check CanBook or similar fields
                    if (prog.CanBook === true || prog.OnlineBooking === true || prog.AllowOnlineBooking === true) {
                        onlineBookablePrograms.add(prog.Id);
                    }
                });
                console.log('📂 All Programs:', Object.values(programs));
                console.log('📂 Online Bookable Program IDs:', Array.from(onlineBookablePrograms));
            }
        } catch (progError) {
        ...
        // Group by ProgramId to see which programs have online-bookable services
        const programsWithOnlineServices = new Set();
        appointmentTypes.forEach(st => {
            if (st.ProgramId) {
                programsWithOnlineServices.add(st.ProgramId);
            }
        });
        console.log('📂 Programs with online services:', Array.from(programsWithOnlineServices).map(id => programs[id]));
        
        // Enrich session types with program name as category
        // Only include services from programs that have at least one online-bookable service
        const enrichedTypes = appointmentTypes
            .filter(st => programsWithOnlineServices.has(st.ProgramId))
            .map(st => ({
                ...st,
                // Use Program name as category (from ProgramId lookup)
                CategoryName: programs[st.ProgramId] || st.Type || 'Tratamientos'
            }));
        ...
        res.json({
            ...
            programs: programs,
            onlinePrograms: Array.from(programsWithOnlineServices).map(id => ({ id, name: programs[id] })),
