...
app.get('/api/bookable-items', async (req, res) => {
    try {
        ...
        console.log('📦 ScheduleItems count:', data.ScheduleItems?.length || 0);
        
        // Check for different response structures
        let scheduleItems = data.ScheduleItems || data.BookableItems || [];
        
        // If ScheduleItems is empty, log more details
        if (scheduleItems.length === 0) {
            console.log('⚠️ No ScheduleItems found. Full response:', JSON.stringify(data, null, 2));
        } else {
        ...
        res.json({
            ...
            ScheduleItems: scheduleItems,  // PascalCase (what widget expects)
            scheduleItems: scheduleItems,  // camelCase (backup)
...
app.get('/api/debug/bookable-items', async (req, res) => {
    try {
        ...
        res.json({
            ...
            firstItem: data.ScheduleItems?.[0] || data.BookableItems?.[0] || null,
            fullResponse: data
        });
