...
app.get('/api/test-services', async (req, res) => {
    try {
        ...
        // Calculate pre-tax prices (Price - TaxIncluded)
...
app.get('/api/session-types', async (req, res) => {
    try {
        ...
        // Fetch prices from /sale/services endpoint
        ...
        try {
            ...
            if (servicesData.Services) {
                ...
                if (servicesData.Services.length > 0) {
                    console.log('💰 First service:', JSON.stringify(servicesData.Services[0], null, 2));
                }
                ...
                servicesData.Services.forEach(svc => {
                    if (svc.Price) {
                        ...
                        console.log(`💰 ${svc.Name}: Price=${svc.Price}, Tax=${taxAmount}, PreTax=${preTaxPrice}`);
                    }
                });
        ...
        // Enrich session types with program name as category and prices from /sale/services
        ...
        // Log sample with price sources
        if (enrichedTypes.length > 0) {
            const sample = enrichedTypes[0];
            console.log('💰 Sample final price:', sample.Name, '=', sample.Price);
        }
...
app.get('/api/bookable-items', async (req, res) => {
    try {
        ...
        // Request more results - Mindbody uses Limit (capital L)
        ...
        while (hasMore && offset < 1000) {  // Safety limit
            ...
            pagedParams.set('Offset', offset.toString());
            
            const pagedEndpoint = `/appointment/bookableitems?${pagedParams.toString()}`;
            console.log(`📅 Fetching page at offset ${offset}...`);
            
            const data = await callMindbodyAPI(pagedEndpoint, 'GET', null, userToken);
            
            const pageItems = data.Availabilities || [];
            ...
            if (pagination) {
                ...
                hasMore = pagination.RequestedOffset + pagination.PageSize < pagination.TotalResults;
                offset += pagination.PageSize || 200;
            } else {
                hasMore = false;
            }
        ...
        // If still empty, try without pagination params
        if (allAvailabilities.length === 0) {
            console.log('⚠️ No results with pagination, trying simple request...');
            const simpleData = await callMindbodyAPI(endpoint, 'GET', null, userToken);
            allAvailabilities = simpleData.Availabilities || simpleData.ScheduleItems || [];
            console.log(`📦 Simple request returned: ${allAvailabilities.length}`);
        }
