...
app.get('/api/session-types', async (req, res) => {
    try {
        ...
        // First, fetch service categories
        let serviceCategories = {};
        try {
            const categoriesData = await callMindbodyAPI('/site/servicecategories', 'GET', null, userToken);
            if (categoriesData.ServiceCategories) {
                categoriesData.ServiceCategories.forEach(cat => {
                    serviceCategories[cat.Id] = cat.Name;
                });
                console.log('📂 Service Categories found:', Object.values(serviceCategories));
            }
        } catch (catError) {
            console.log('⚠️ Could not fetch service categories:', catError.message);
        }
        ...
        try {
            ...
            if (programsData.Programs) {
                ...
                console.log('📂 Programs found:', Object.values(programs));
            }
        } catch (progError) {
        ...
        if (appointmentTypes.length > 0) {
            console.log('📋 Sample session type structure:', JSON.stringify(appointmentTypes[0], null, 2));
        }
        
        // Enrich session types with category names from multiple sources
        const enrichedTypes = appointmentTypes.map(st => ({
            ...st,
            // Try multiple sources for category name
            CategoryName: st.Category?.Name || 
                          serviceCategories[st.CategoryId] || 
                          serviceCategories[st.ServiceCategoryId] ||
                          st.Subcategory?.Name ||
                          programs[st.ProgramId] ||
                          null
        }));
