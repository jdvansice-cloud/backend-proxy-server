...
app.get('/api/test-pricing-options', async (req, res) => {
    try {
        ...
        // Filter for single session options
        const singleSessionOptions = allOptions.filter(opt => 
            opt.NumberOfSessions === 1 || opt.Count === 1
        );
        
        console.log('💰 Single session options:', singleSessionOptions.length);
        
        // Show sample with session type mappings
        const sampleOptions = singleSessionOptions.slice(0, 10).map(opt => ({
            Id: opt.Id,
            Name: opt.Name,
            Price: opt.Price,
            OnlinePrice: opt.OnlinePrice,
            NumberOfSessions: opt.NumberOfSessions,
            Count: opt.Count,
            SessionTypeIds: opt.SessionTypeIds
        }));
        ...
        res.json({
            ...
            sampleSingleSessionOptions: sampleOptions,
            rawFirstOption: allOptions[0] || null
        });
...
app.get('/api/test-available-dates', async (req, res) => {
    try {
        ...
        params.append('endDate', thirtyDaysLater.toISOString().split('T')[0]);
...
app.get('/api/session-types', async (req, res) => {
    try {
        ...
        // Fetch pricing options to get single-session prices
        let pricingBySessionType = {};
        try {
            const pricingData = await callMindbodyAPI('/sale/pricingoptions', 'GET', null, userToken);
            if (pricingData.PricingOptions) {
                console.log('💰 Total pricing options:', pricingData.PricingOptions.length);
                
                // Filter for single-session pricing options (NumberOfSessions = 1)
                pricingData.PricingOptions.forEach(option => {
                    // Look for single session options
                    if (option.NumberOfSessions === 1 || option.Count === 1) {
                        // Map to session types this pricing applies to
                        if (option.SessionTypeIds && option.SessionTypeIds.length > 0) {
                            option.SessionTypeIds.forEach(stId => {
                                // Only set if we don't have a price yet, or this is a lower price
                                if (!pricingBySessionType[stId] || option.OnlinePrice < pricingBySessionType[stId]) {
                                    pricingBySessionType[stId] = option.OnlinePrice || option.Price;
                                }
                            });
                        }
                    }
                });
                console.log('💰 Single-session prices found for', Object.keys(pricingBySessionType).length, 'session types');
                console.log('💰 Sample prices:', Object.entries(pricingBySessionType).slice(0, 5));
            }
        } catch (pricingError) {
            console.log('⚠️ Could not fetch pricing options:', pricingError.message);
        }
        ...
        if (appointmentTypes.length > 0) {
            ...
            console.log('💰 Sample session type - from Pricing:', pricingBySessionType[appointmentTypes[0].Id]);
        }
        ...
        const enrichedTypes = appointmentTypes
            .map(st => ({
                ...
                // Get price from pricing options (single session)
                Price: pricingBySessionType[st.Id] || st.OnlinePrice || st.DefaultPrice || st.Price || null
            }))
        ...
        res.json({
            ...
            programs: programs,
            pricingBySessionType: pricingBySessionType,
...
app.get('/api/available-dates', async (req, res) => {
    try {
        ...
        // Default to today + 30 days (API limit) if not specified
