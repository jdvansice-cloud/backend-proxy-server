 ...
 * 8. Book Appointment
 * 
 * APPOINTMENT REQUEST MODE (NEW - November 2024):
 * The AddAppointment endpoint now supports submitting appointments as "requests" 
 * rather than direct bookings. This creates a pending request that staff must 
 * approve before it appears on the calendar.
 * 
 * Set isRequest: true to use appointment request mode.
 * This is controlled by the APPOINTMENT_REQUEST_MODE environment variable.
 */
app.post('/api/appointments/book', async (req, res) => {
    try {
        ...
        // NEW: Appointment Request Mode
        // Can be set per-request or globally via environment variable
        // When true, creates a pending request instead of confirmed booking
        const isRequest = req.body.isRequest !== undefined 
            ? req.body.isRequest 
            : (process.env.APPOINTMENT_REQUEST_MODE === 'true');
        
        console.log('📅 ' + (isRequest ? 'Creating appointment REQUEST:' : 'Booking appointment:'));
        console.log('   Client:', clientId);
        console.log('   SessionType:', sessionTypeId);
        console.log('   Staff:', staffId);
        console.log('   Location:', locationId);
        console.log('   DateTime:', startDateTime);
        console.log('   Mode:', isRequest ? '📋 REQUEST (pending approval)' : '✅ DIRECT BOOKING');
        ...
        // Add appointment request flag if enabled
        // Mindbody API parameter for appointment requests (added November 2024)
        if (isRequest) {
        ...
        if (isRequest) {
            console.log('📋 Appointment REQUEST created! ID:', data.Appointment?.Id);
        } else {
