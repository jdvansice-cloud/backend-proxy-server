...
app.post('/api/clients/login', async (req, res) => {
    try {
        ...
        console.log('   Using staff token:', userToken ? 'Yes' : 'No');
        ...
        // Try to validate client credentials
        console.log('   Calling validateclientcredentials...');
        const response = await axios.post(
            `${CONFIG.baseUrl}/client/validateclientcredentials`,
            {
        ...
        let userMessage = 'Email o contraseña incorrectos';
        
        if (errorMessage.includes('not found') || errorMessage.includes('Invalid')) {
            userMessage = 'No se encontró una cuenta con este email. ¿Necesitas crear una cuenta?';
        } else if (errorMessage.includes('password') || errorMessage.includes('credential')) {
            userMessage = 'Contraseña incorrecta. Usa "¿Olvidaste tu contraseña?" para recuperarla.';
        }
