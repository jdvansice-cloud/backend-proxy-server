...
app.get('/api/session-types', async (req, res) => {
    try {
        ...
        if (appointmentTypes.length > 0) {
            ...
            console.log('📋 Session type fields:', Object.keys(appointmentTypes[0]));
        }
...
app.get('/api/bookable-items', async (req, res) => {
    try {
        ...
        // Fetch with pagination - collect all pages
        ...
        do {
            ...
            const endpoint = `/appointment/bookableitems?${pagedParams.toString()}`;
            ...
            if (data.PaginationResponse) {
                ...
                console.log(`📄 Pagination: ${offset + pageItems.length}/${totalResults}`);
            }
            
            offset += limit;
            
            // Stop if we got less than limit (no more pages) or reached total
        } while (allAvailabilities.length < totalResults && offset < 1000);
