...
app.get('/', (req, res) => {
    res.json({
        ...
        endpoints: {
            ...
            staff: 'GET /api/staff',
            staffAvailability: 'GET /api/staff/availability',
            bookableItems: 'GET /api/bookable-items',
            bookingRequest: 'POST /api/booking-request',
...
// 3. Get Session Types (Services/Treatments)
app.get('/api/session-types', async (req, res) => {
    try {
        const userToken = req.headers.authorization;
        const locationId = req.query.locationId;

        let endpoint = '/site/sessiontypes?OnlineOnly=true'; // Only show online bookable services
        if (locationId) {
...
// 4. Get Staff with Location Filter
...
// 4b. Get Staff Availability for next 2 weeks
app.get('/api/staff/availability', async (req, res) => {
    try {
        const userToken = req.headers.authorization;
        const { staffIds, locationId, startDate, endDate } = req.query;
        ...
        const endpoint = `/staff/staffavailability?${params.toString()}`;
        
        console.log('Fetching staff availability:', params.toString());
        const data = await callMindbodyAPI(endpoint, 'GET', null, userToken);
        console.log('Staff availability fetched');
        res.json(data);
    } catch (error) {
...
// 5. Get Bookable Items (Available Appointment Slots)
...
// 6. Create Booking Request (Add Appointment)
app.post('/api/booking-request', async (req, res) => {
    try {
        const userToken = req.headers.authorization;
        const bookingData = req.body;

        // Ensure the appointment is set as a request (not confirmed)
        if (!bookingData.Test) {
            bookingData.Test = false; // Set to true for testing
        }

        console.log('Creating booking request:', JSON.stringify(bookingData, null, 2));
